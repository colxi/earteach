		...
		*/
		...
		makeScale: function(mode,name,root){
			...
			*/
			...
			for(var i = 0; i<8; i++){
				scaleUp.push(this.makeIntervalNote(i+1,this.def.scales[mode][name][0][i],'asc',root));
			};
			
			// generate scale descending
			var scaleDown = new Array();
			for(var i = 0; i<8; i++){
				scaleDown.push(this.makeIntervalNote(i+1,this.def.scales[mode][this.def.scales[mode][name][1]][0][i],'asc',root));
			};
			scaleDown.reverse();
