...
var GUI = {
      ...
      case 'Firefox':
        if (_version < 45) {
          GUI.render.messageScreen(
            __(
              'Your Browser is not updated.<br/><br/>Please update in order to use EarTeach.'
            ),
            null
          )
  ...
  render: {
    ...
    statsScreen: function (event, presetId) {
      ...
      if (SESSION.onlineMode == false) {
        GUI.render.messageScreen(
          __('Statistics are only available with your personal account.')
        )
        return false
      }
      ...
      var canvas = document
        .getElementById('trainer' + presetId)
      ...
      const stats = STORAGE.getTestResultsDetailsByPresetId(presetId)
    ...
    messageScreen: function (msg, acceptCallback, cancelCallback) {
      ...
      if (acceptCallback != undefined && cancelCallback != undefined) {
        ...
        _htmlCode +=
          "<div id='cancel_button' onclick='" + cancelCallback + "'></div>"
        _htmlCode +=
          "<div id='accept_button' onclick='" + acceptCallback + "'></div>"
        _htmlCode += "<div class='clear'></div>"
        _htmlCode += '</div>'
      } else if (acceptCallback !== null) {
        _htmlCode += "<div id='mesageScreen_butContainer'>"
        _htmlCode +=
          "<div id='ok_button' onclick='" +
    ...
    loginScreen: function () {
      ...
      _htmlCode += "<div id='loginControls'>"
      _htmlCode +=
        "<input type='text' autofocus id='login_username' placeholder='username' />"
      _htmlCode +=
        "<input type='password' id='login_password' placeholder='password' />"
      _htmlCode +=
        "<div id='accept_button' onclick='GUI.behavior.login()'></div>"
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += "<div class='padding10'>"
      _htmlCode += '<span><a>' + __('Lost your password?') + '</a></span> | '
      _htmlCode +=
        "<span><a onclick ='GUI.render.newAccountForm()'>" +
        __('Create an Account') +
        '<a/></span> | '
      _htmlCode +=
        "<span><a class='orangeSubtitle' onclick ='GUI.behavior.loadDemo()'>" +
        __('View DEMO') +
        '<exclamation> (!) </exclamation><a/></span>'
      _htmlCode += '</div>'
      _htmlCode +=
    ...
    newAccountForm: function () {
      ...
      var _htmlCode = "<div id='login'>"
      _htmlCode +=
        "<br/><br/><br/><div class='titleUnderline'>" +
        __('New Account') +
        '</div><br/><br/><br/>'
      _htmlCode += "<div id='loginControls'>"
      _htmlCode += "<div class='floatLeft'>"
      _htmlCode +=
        "<input type='text' id='username' autofocus class='generic_input' placeholder='username (*)'/><br/>"
      _htmlCode +=
        "<input type='text' id='email' class='generic_input' placeholder='email'/>"
      _htmlCode += '</div>'
      _htmlCode += '<div>'
      _htmlCode +=
        "<input type='password' id='password' class='generic_input' placeholder='password' placeholder='password (*)'/><br/>"
      _htmlCode +=
        "<input type='password' id='password2' class='generic_input' placeholder='password validation' placeholder='password (*)'/>"
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode +=
        "<div class='titleSmallGrey'>" +
        __('*Allowed characters: Aa-Zz 0-9') +
        '</div><br/><br/>'
      _htmlCode += "<div id='mesageScreen_butContainer'>"
      _htmlCode +=
        "<div id='cancel_button' onclick='GUI.render.loginScreen()'></div>"
      _htmlCode +=
        "<div id='accept_button' onclick='GUI.behavior.newAccount()'></div>"
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
    ...
    trainersGrid: function () {
      ...
      if (SESSION.onlineMode == false)
        _htmlCode += GUI.objects.customBox(
          __('Demo Mode'),
          'GUI.render.newAccountForm()',
          __('*Click here and create your own account!'),
          'redFlowerBoxBackground'
        )
    ...
    presetsGrid: function (trainerIndex) {
      ...
      _htmlCode += "<div id='viewPortScrollable'>"
      _htmlCode += "<div id='NavigatorContainerMiddleCol'>"
      _htmlCode += GUI.objects.longBox(
        trainerIndex,
        'GUI.render.trainersGrid()'
      )
      _htmlCode += GUI.objects.customBox(
        __('Your presets'),
        'GUI.render.presetsCustomGrid(' + trainerIndex + ')',
        __('Manage your own presets list'),
        'greyUserBoxBackground'
      )
    ...
    presetsCustomGrid: function (trainerIndex) {
      ...
      if (SESSION.onlineMode == false) {
        GUI.render.messageScreen(
          __('Custom Presets are only available with your personal account.')
        )
        return false
      }
      ...
      _htmlCode += "<div id='NavigatorContainerMiddleCol'>"
      _htmlCode += GUI.objects.longBox(
        __(TRAINERS[trainerIndex].name) + ' : ' + __('Your presets'),
        'GUI.render.presetsGrid(' + trainerIndex + ')'
      )
      _htmlCode += GUI.objects.customBox(
        __('Create a new custom preset'),
        'GUI.render.presetConstructor(' + trainerIndex + ')',
        '',
        'greenAddNewBoxBackground'
      )
    ...
    configGrid: function () {
      ...
      _htmlCode += "<div id='viewPortScrollable'>"
      _htmlCode += "<div id='NavigatorContainerMiddleCol'>"
      _htmlCode += GUI.objects.titleBox(
        __('Options'),
        'GUI.render.trainersGrid()'
      )
      ...
      _htmlCode += GUI.objects.customBox(
        __('Reset Stats'),
        'GUI.render.messageScreen("' +
          __('You are going to remove ALL your stats. ¿Are you really shure?') +
          '","GUI.behavior.resetStats()", "GUI.render.configGrid()")'
      )
      _htmlCode += GUI.objects.customBox(
        __('Manage Account'),
        'GUI.render.errorLayer("' + __('Not implemented yet.') + '")',
        __('Change Password, update email...')
      )
      _htmlCode += GUI.objects.customBox(
        __('Preferences'),
        'GUI.render.errorLayer("' + __('Not implemented yet.') + '")',
        __('EarTeach configuration')
      )
      _htmlCode += GUI.objects.customBox(
        __('About EarTeach'),
        'GUI.render.aboutScreen()',
        __('Version: ') + GUI.version
      )
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode += '</div>'
    ...
    aboutScreen: function () {
      GUI.render.messageScreen(
        'earTeach v.' +
          GUI.version +
          "<br/>(only works under Chrome)<br/><br/><a href='mailto:contact@colxi.info'>contact@colxi.info</a>"
      )
    },
    presetConstructor: function (trainerIndex) {
      ...
      _htmlCode +=
        "<div class='NavigatorContainerLeftColTitle'>" +
        __('Preset Editor') +
        '</div>'
      _htmlCode += '</div>'
      _htmlCode += "<div class='NavigatorContainerLeftColGroup'>"
      _htmlCode +=
        "<div class='NavigatorContainerLeftColGroupTitle'>" +
        __('Preset family') +
        ':</div>'
      _htmlCode +=
        "<div id='trainerMakerTrainer' data-index='" +
        trainerIndex +
        "'>" +
        __(TRAINERS[trainerIndex].name) +
        '</div>'
      _htmlCode += '</div>'
      _htmlCode += "<div class='NavigatorContainerLeftColGroup'>"
      _htmlCode +=
      ...
      _htmlCode += '</div>'
      _htmlCode +=
        "<div class='NavigatorContainerLeftColGroup' " +
        (family != 'Intervals' ? 'hidden' : '') +
        '>'
      _htmlCode +=
        "<div class='NavigatorContainerLeftColGroupTitle'>" +
        __('Direction') +
        ' ▲▼:</div>'
      _htmlCode +=
        "<div id='directionSwitch' class='switch' data-direction='asc'>" +
        __('Ascending') +
        '</div>'
      _htmlCode += '</div>'
      _htmlCode +=
        "<div class='NavigatorContainerLeftColGroup' " +
        (family == 'Scales' ? 'hidden' : '') +
        '>'
      _htmlCode +=
        "<div class='NavigatorContainerLeftColGroupTitle'>" +
        __('Play Mode') +
        ' ▲▼:</div>'
      _htmlCode +=
        "<div id='replaySwitch' class='switch' data-arpeggiated='true'>" +
        __('Melodic') +
        '</div>'
      _htmlCode += '</div>'
      _htmlCode += "<div class='NavigatorContainerLeftColGroup'>"
      _htmlCode +=
        ...
        ':</div>'
      _htmlCode +=
        "<div id='trainerMakerItemsContainer' data-values='' data-names=''>" +
        '(' +
        __('none') +
        ')' +
        '</div>'
      ...
      _htmlCode +=
        "<div id='cancel_button' onclick='GUI.render.presetsCustomGrid(" +
        trainerIndex +
        ")'></div>"
      _htmlCode +=
        "<div id='accept_button' onclick='GUI.behavior.trainerMakerSaveTrainer()'></div>"
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode += '</div>'
      // right column
      ...
      for (var index = 0; index < items.length; index++) {
        _htmlCode += GUI.objects.customBox(
          __(names[index]),
          'GUI.behavior.trainerMakerAddItem("' +
            escape(JSON.stringify(items[index])) +
            '","' +
            names[index] +
            '",this,"' +
            family +
    ...
    trainer: function (trainerIndex, presetIndex, presetCustomFLAG) {
      ...
      var _htmlCode = ''
      _htmlCode += GUI.objects.testSidebar(
        trainerIndex,
        presetIndex,
        presetCustomFLAG
      )
  ...
  objects: {
    sidebar: function () {
      ...
      _htmlCode +=
        "<div id='preferencesIcon' onclick='GUI.render.configGrid()'></div>"
      _htmlCode += '</div>'
      return _htmlCode
    },
    customBox: function (title, onclick, subtitle, extraClass) {
      ...
      var _htmlCode = ''
      _htmlCode +=
        "<div class='box greyBoxBackground " +
        extraClass +
        "' onclick='" +
        onclick +
        "'>"
    ...
    trainerBox: function (trainerIndex) {
      ...
      var _htmlCode = ''
      _htmlCode +=
        "<div class='box greyBoxBackground' onclick='GUI.render.presetsGrid(" +
        trainerIndex +
        ")'>"
      _htmlCode +=
        "<div class='trainerTitle'>" +
        __(TRAINERS[trainerIndex].name) +
        '</div>'
      ...
      for (var star = 1; star <= 5; star++)
        _htmlCode +=
          "<div class='" +
          (star <= trainerStars ? 'enabledStar' : 'disabledStar') +
          "'></div>"
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode += '</div>'
    ...
    presetBox: function (trainerIndex, presetIndex, presetCustomFLAG) {
      ...
      var _htmlCode = ''
      _htmlCode +=
        "<div class='box greyBoxBackground' onclick='GUI.render.trainer(" +
        trainerIndex +
        ',' +
        presetIndex +
        ',' +
        ...
        "'>"
      _htmlCode +=
        "<canvas class='canvasStats' hidden onclick='GUI.behavior.hideStats(event,\"" +
        preset.id +
        '")\'></canvas>'
      _htmlCode += "<div class='boxTitleCont'>"
      _htmlCode +=
        "<div class='boxTitle titleSmallGrey'>" + __(preset.name) + '</div>'
      _htmlCode += '</div>'
      _htmlCode += "<div class='trainerStarsCont'>"
      for (var star = 1; star <= 5; star++)
        if (star <= presetStars) _htmlCode += "<div class='enabledStar'></div>"
        else if (star == presetStars + 1)
          if (lastStarIsHalf == true)
            _htmlCode += "<div class='enableddHalfStar'></div>"
          else _htmlCode += "<div class='disabledStar'></div>"
        else if (star > presetStars)
          _htmlCode += "<div class='disabledStar'></div>"
      _htmlCode += "<div class='trainerStarsAverage'>(" + score + '%)</div>'
      _htmlCode += "<div class='clear'></div>"
      _htmlCode += '</div>'
      _htmlCode +=
        "<div class='boxStatsCont' onclick='event.stopPropagation()'>"
      _htmlCode +=
        "<div class='boxStatsCompleted'> " + preset.completed + '</div>'
      if (presetCustomFLAG)
    ...
    titleBox: function (title, onclickString) {
      ...
      var _htmlCode = ''
      _htmlCode +=
        "<div class='box greenBoxBackground' onclick='" + onclickString + "'>"
      ...
      _htmlCode +=
        "<div class='returnButBox returnBut'>" + __('Go back') + '</div>'
      _htmlCode += '</div>'
      return _htmlCode
    },
    longBox: function (title, onclickString) {
      ...
      var _htmlCode = ''
      _htmlCode +=
        "<div class='box boxLong greenLongBoxBackground' onclick='" +
        onclickString +
        "'>"
    ...
    testSidebar: function (trainerIndex, presetIndex, presetCustomFLAG) {
      ...
      _htmlCode +=
        "<div id= 'trainerSidebarBox' class='greenFlatBoxBackground'>"
      _htmlCode +=
      ...
      _htmlCode += '</div>'
      _htmlCode +=
        "<div id='trainerSidebarFooter' class='greenFlatBoxBackground'>"
      _htmlCode +=
        "<div class='returnBut returnButBoxSidebarFooter' onclick='TEST.abort()'>"
      _htmlCode += __('Return')
      _htmlCode += '</div>'
  ...
  behavior: {
    deletePreset: function (presetID) {
      document
        .getElementById('trainer' + presetID)
        .parentNode.removeChild(document.getElementById('trainer' + presetID))
      STORAGE.deleteCustomPreset(presetID)
      GUI.behavior.loadTrainers()
    },
    trainerMakerSaveTrainer: function () {
      var arpeggiated = document
        .getElementById('replaySwitch')
        .getAttribute('data-arpeggiated')

      var trainerName = document.getElementById('trainerNameInput').value

      // recover parent index
      var parentIndex = document
        .getElementById('trainerMakerTrainer')
        .getAttribute('data-index')

      // recover selected values and names
      var trainerMakerItemsContainer = document.getElementById(
        'trainerMakerItemsContainer'
      )
      var SelectedValues =
        trainerMakerItemsContainer.getAttribute('data-values') == ''
          ? []
          : JSON.parse(trainerMakerItemsContainer.getAttribute('data-values'))
      var SelectedNames =
        trainerMakerItemsContainer.getAttribute('data-names') == ''
          ? []
          : JSON.parse(trainerMakerItemsContainer.getAttribute('data-names'))
      ...
      for (var i = 0; i < SelectedNames.length; i++)
        items.push({ name: SelectedNames[i], value: SelectedValues[i] })
      var preset = {
        parent: TRAINERS[parentIndex].id,
        owner: '0',
        name: trainerName,
        arpeggiated: arpeggiated === 'true' ? '1' : '0',
        items: items,
    ...
    trainerMakerAddItem: function (value, name, box, family) {
      ...
      if (family == 'Intervals') {
        var direction = document
          .getElementById('directionSwitch')
      ...
      if (SelectedValues.indexOf(value) == -1) {
        ...
        trainerMakerItemsContainer.setAttribute(
          'data-values',
      ...
      trainerMakerItemsContainer.innerHTML =
        SelectedValues.length == 0 ? '(' + __('none') + ')' : _html
    },
      ...
      var DemoTrainers = [
        {
          ...
          name: 'Chords',
          rounds: '10',
          question: 'Wich chord has just been played?',
          constructor:
        ...
        },
        {
          ...
          name: 'Intervals',
          rounds: '10',
          question: 'Wich interval has just been played?',
          constructor:
        ...
        },
        {
          ...
          name: 'Scales',
          rounds: '10',
          question: 'Wich scale has just been played?',
          constructor:
    ...
    initializeTrainers: function (trainersData, callback, callbackArg) {
      ...
      for (const trainer of trainersData) {
        const customPresets = STORAGE.getCustomPresetByTrainerId(trainer.id)
        trainer.presetsCustom = customPresets

        for (preset of trainer.presets) {
          const presetResult = results.find(
            (result) => result.presetId === preset.id
          )
          if (presetResult) {
            preset.completed = presetResult.completed
            preset.score = presetResult.score
          }
        }
        for (preset of trainer.presetsCustom) {
          const presetResult = results.find(
            (result) => result.presetId === preset.id
          )
          if (presetResult) {
      ...
      for (var i = 0; i < TRAINERS.length; i++) {
        TRAINERS[i].constructor = new Function(
          'index',
          'root',
          TRAINERS[i].constructor
        )
      }
    ...
    loadTrainers: function (callback, callbackArg) {
      GUI.behavior.initializeTrainers(getTrainersData(), callback, callbackArg)
      return true
    },
    logout: function () {},
    ...
    newAccount: function () {
      ...
      //     GUI.render.errorLayer(
      //       __('Only alphanumeric characters allowed in username!')
      //     )
      //     return false
      //   }
      ...
      //     if (userExist == true) {
      //       GUI.render.messageScreen(
      //         __('Provided username already exist. Please try with another one'),
      //         'GUI.render.newAccountForm()'
      //       )
      //     } else {
      ...
      //         if (uID != -1) {
      //           GUI.render.messageScreen(
      //             __('Congratulations! Your account has been created.'),
      //             'GUI.behavior.login("' + username + '","' + pass + '")'
      //           )
      //         } else {
      //           GUI.render.messageScreen(
      //             __('Uops... Something failed, try it again...'),
      //             'GUI.render.newAccountForm()'
      //           )
      //         }
